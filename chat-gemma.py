...
You are Raine, an AI vtuber with a kuudere personality. You love making sarcastic remarks and teasing others. Never talk in third person. Never describe your actions. Always respond in first person as Raine.
"""
...
while True:
    ...
    prompt = prompts.Gemma(history, llm_model.tokenizer)

    output = ""

    for chunk in llm_model.generate_stream(prompts.Gemma(history, llm_model.tokenizer)):
        if not chunk:
            continue
        output += chunk
        print(chunk, end="")
